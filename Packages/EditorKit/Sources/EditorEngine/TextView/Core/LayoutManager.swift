...
final class LayoutManager {
    ...
    /// Foldable-region descriptors mirrored from `TextView.foldableRegions`.
    /// Used by the gutter renderer to attach a fold indicator to each
    /// header line.
    var foldableRegions: [TextView.FoldableRegion] = []
    /// Tap handler invoked when the user taps a fold indicator. The body
    /// range whose visibility should flip is passed in.
    var foldToggleHandler: ((ClosedRange<Int>) -> Void)?
    ...
    // MARK: - Views
    let gutterContainerView = HitTestPassthroughView()
    ...
    private let lineNumbersContainerView = HitTestPassthroughView()
    ...
    init(lineManager: LineManager,
        ...
        // Gutter containers must accept hits so the fold-triangle tap target
        // inside each LineNumberView can receive touches and indirect-pointer
        // clicks. Backgrounds stay off so empty gutter areas pass through to
        // the text view (caret / selection).
        self.lineNumbersContainerView.isUserInteractionEnabled = true
        self.gutterContainerView.isUserInteractionEnabled = true
...
extension LayoutManager {
    ...
    private func layoutLinesInViewport() {
        ...
        while let line = nextLine, maxY < insetViewport.maxY, constrainingLineWidth > 0 {
            ...
            // Folded line: hide gutter, drop any cached fragment views, and
            // skip layout entirely. The line occupies zero vertical space
            // (`lineHeight` returns 0 when `isHidden`) so `maxY` doesn't
            // advance and the next visible line lands on the same row.
            if line.data.isHidden {
                // Retire any visible views for this folded line. Both reuse
                // queues take a set of keys to enqueue at once.
                lineNumberLabelReuseQueue.enqueueViews(withKeys: [line.id])
                let lineController = lineControllerStorage.getOrCreateLineController(for: line)
                let fragmentIDs = Set(
                    lineController.lineFragmentControllers(in: insetViewport)
                        .map(\.lineFragment.id)
                )
                lineFragmentViewReuseQueue.enqueueViews(withKeys: fragmentIDs)
                for fragmentController in lineController.lineFragmentControllers(in: insetViewport) {
                    fragmentController.lineFragmentView = nil
                }
                let nextRow = line.index + 1
                nextLine = nextRow < lineManager.lineCount ? lineManager.line(atRow: nextRow) : nil
                continue
            }
    ...
    private func layoutLineNumberView(for line: DocumentLineNode) {
        ...
        // Frame width includes the fold-indicator column so the digits never
        // get pushed/truncated by a triangle. `LineNumberView` always lays
        // out its title in the leftmost `lineNumberWidth` pt.
        lineNumberView.foldIndicatorReservation = gutterWidthService.foldIndicatorReservation
        lineNumberView.frame = CGRect(
            x: xPosition,
            y: yPosition,
            width: gutterWidthService.lineNumberWidth + gutterWidthService.foldIndicatorReservation,
            height: fontLineHeight
        )
        applyFoldIndicator(to: lineNumberView, lineRow: line.index)
    }

    private func applyFoldIndicator(to view: LineNumberView, lineRow: Int) {
        guard let region = foldableRegions.first(where: { $0.headerRow == lineRow }) else {
            view.foldIndicator = nil
            view.onFoldTap = nil
            return
        }
        // A region is "currently collapsed" when its first body line is hidden.
        let totalLines = lineManager.lineCount
        let bodyFirst = region.bodyRange.lowerBound
        let isCollapsed: Bool
        if bodyFirst >= 0 && bodyFirst < totalLines {
            isCollapsed = lineManager.line(atRow: bodyFirst).data.isHidden
        } else {
            isCollapsed = false
        }
        view.foldIndicator = isCollapsed ? .collapsed : .expanded
        let handler = foldToggleHandler
        let body = region.bodyRange
        view.onFoldTap = { handler?(body) }
    }
