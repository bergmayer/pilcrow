...
final class TextInputView: UIView, UITextInput {
    ...
    init(theme: Theme) {
        ...
        // Light/dark switch → invalidate so the theme palette re-applies.
        // Replaces the deprecated `traitCollectionDidChange` override.
        registerForTraitChanges([UITraitUserInterfaceStyle.self]) { (view: TextInputView, _) in
            view.invalidateLines()
            view.layoutManager.setNeedsLayout()
        }
    }
    ...
    /// App-supplied foldable-region descriptors, mirrored from the public
    /// `TextView.foldableRegions`. Read by the gutter renderer to draw
    /// triangle indicators on header lines.
    var foldableRegions: [TextView.FoldableRegion] = [] {
        didSet {
            layoutManager.foldableRegions = foldableRegions
        }
    }

    /// Closure routed from `TextView.onFoldToggle`. The gutter invokes it
    /// when the user taps a fold triangle.
    var foldToggleHandler: ((ClosedRange<Int>) -> Void)? {
        didSet {
            layoutManager.foldToggleHandler = foldToggleHandler
        }
    }

    /// Post-syntax-highlight attribute decorator. Called for each line
    /// after the language mode's highlighter has run. Use it to layer on
    /// extra attributes (foreground color, font traits) without touching
    /// tree-sitter — see `TextView.attributeDecorator`.
    var attributeDecorator: ((NSMutableAttributedString, Int, Int) -> Void)? {
        didSet {
            // Force a re-highlight pass so the new decorator runs over
            // already-laid-out lines.
            lineControllerStorage.removeAllLineControllers()
            layoutManager.setNeedsLayout()
            setNeedsLayout()
        }
    ...
    /// Trigger relayout after fold state on `LineManager` has changed.
    /// Drops the line-controller cache so previously-typeset lines re-emit
    /// with their new (zero when hidden) height, then nudges the layout
    /// pipeline.
    func invalidateLayoutForFoldChange() {
        lineControllerStorage.removeAllLineControllers()
        contentSizeService.invalidateContentSize()
        layoutManager.setNeedsLayout()
        setNeedsLayout()
...
extension TextInputView {
    ...
    func deleteBackward() {
        ...
        // Disable notifying delegate in layout subviews to prevent sending the selected range with length > 0 when deleting text. This aligns with the behavior of UITextView and was introduced to resolve issue #158: the underlying UITextView behavior
        notifyDelegateAboutSelectionChangeInLayoutSubviews = false
...
extension TextInputView: LineControllerDelegate {
    ...
    func lineControllerDecorate(
        ...
        attributeDecorator?(attributedString, lineLocation, lineLength)
    }
}
