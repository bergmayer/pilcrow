...
open class TextView: UIScrollView {
    ...
    open override var accessibilityValue: String? {
        get { text }
        set { /* read-only mirror of `text` */ _ = newValue }
    }
    ...
    ///
    /// The `softLineBreakSymbol` is used to render line breaks. These line breaks are typically represented by the U+2028 unicode character. EditorEngine does not provide any key commands for inserting these but supports rendering them.
    ...
    /// A folded-region descriptor: a `headerRow` (visible "opener" line)
    /// plus the body `bodyRange` of lines that collapse when folded.
    public struct FoldableRegion: Equatable, Sendable {
        public let headerRow: Int
        public let bodyRange: ClosedRange<Int>
        public init(headerRow: Int, bodyRange: ClosedRange<Int>) {
            self.headerRow = headerRow
            self.bodyRange = bodyRange
        }
    }

    /// All foldable regions in the current document. Update this whenever
    /// the buffer or language changes — the gutter uses it to draw fold
    /// triangles on header lines.
    public var foldableRegions: [FoldableRegion] = [] {
        didSet {
            textInputView.foldableRegions = foldableRegions
            textInputView.setNeedsLayout()
        }
    }

    /// Invoked when the user taps a fold-indicator triangle in the gutter.
    /// The single argument is the body range whose visibility should flip.
    public var onFoldToggle: ((ClosedRange<Int>) -> Void)? {
        didSet { textInputView.foldToggleHandler = onFoldToggle }
    }

    /// Post-syntax-highlight attribute decorator. Called once per line
    /// after the language-mode highlighter has run, with the line's
    /// `NSMutableAttributedString`, its absolute starting UTF-16 offset
    /// in the document, and its length. The closure can apply
    /// `NSAttributedString.Key`-keyed attributes (foreground color, font,
    /// underline) — they layer cleanly on top of the tree-sitter pass.
    ///
    /// Use this when you need foreground-colored highlights that aren't
    /// expressible as tree-sitter captures — e.g. inline Markdown
    /// patterns when the block grammar is doing block-level highlights.
    public var attributeDecorator: ((NSMutableAttributedString, Int, Int) -> Void)? {
        didSet { textInputView.attributeDecorator = attributeDecorator }
    }

    /// Mark a range of line indices (0-based, inclusive) as folded — they
    /// disappear from layout while remaining present in the document text
    /// (so undo / find / encoding / save / tree-sitter all keep seeing the
    /// real content). Pass `folded: false` to expand the range again.
    ///
    /// Typical use: the editor host computes a body range that follows a
    /// header line (e.g. lines 6...14 of a function whose `{` is on line 5)
    /// and calls `setLinesFolded(true, range: 6...14)`. Line 5 stays
    /// visible; the body collapses.
    public func setLinesFolded(_ folded: Bool, range: ClosedRange<Int>) {
        let lineCount = textInputView.lineManager.lineCount
        let clamped = max(0, range.lowerBound)...min(lineCount - 1, range.upperBound)
        guard clamped.lowerBound <= clamped.upperBound else { return }
        var anyChanged = false
        for index in clamped {
            let line = textInputView.lineManager.line(atRow: index)
            if textInputView.lineManager.setHidden(folded, of: line) {
                anyChanged = true
            }
        }
        guard anyChanged else { return }
        // Drop cached heights on the affected controllers so the layout
        // pass sees zero height instead of the previous typeset height.
        textInputView.invalidateLayoutForFoldChange()
        // If the cursor is inside a freshly-hidden range, snap it to the
        // line just before the fold so the caret remains visible.
        if folded {
            let firstLine = textInputView.lineManager.line(atRow: clamped.lowerBound)
            let lastLine  = textInputView.lineManager.line(atRow: clamped.upperBound)
            let hiddenRange = NSRange(
                location: firstLine.location,
                length: lastLine.location + lastLine.data.totalLength - firstLine.location
            )
            if let selected = textInputView.selectedRange,
               NSIntersectionRange(selected, hiddenRange).length > 0 || (selected.location > firstLine.location && selected.location < hiddenRange.location + hiddenRange.length) {
                let safeLocation = max(0, firstLine.location)
                textInputView.selectedRange = NSRange(location: safeLocation, length: 0)
            }
        }
    }

    /// Convenience that unfolds every folded line.
    public func unfoldAllLines() {
        let total = textInputView.lineManager.lineCount
        guard total > 0 else { return }
        var anyChanged = false
        for index in 0..<total {
            let line = textInputView.lineManager.line(atRow: index)
            if line.data.isHidden {
                if textInputView.lineManager.setHidden(false, of: line) {
                    anyChanged = true
                }
            }
        }
    ...
    /// Returns the 0-based line indices of every currently-hidden line.
    public var foldedLineIndices: [Int] {
        var indices: [Int] = []
        let total = textInputView.lineManager.lineCount
        for index in 0..<total {
            if textInputView.lineManager.line(atRow: index).data.isHidden {
...
extension TextView {
    ...
    /// Caret rect at a raw character location, in this view's
    /// coordinate space (i.e. content-coordinate; scrolling already
    /// accounted for via `contentOffset` like any other UIScrollView
    /// subview). Returned rect's `minY` is the top of the line and its
    /// `height` matches the line's typeset height — enough to align
    /// per-line decorations such as bookmark badges in the gutter.
    public func caretRect(atCharacterIndex location: Int) -> CGRect {
...
extension TextView: TextInputViewDelegate {
    ...
    func textInputViewDidChangeSelection(_ view: TextInputView) {
        // UIEditMenuInteraction dismisses itself on selection change; no
        // explicit hide call is required on iOS 16+.
